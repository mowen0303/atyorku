...
//        checkAuthority('get_many', $sectionId, $reviewStatus);
        $result = $videoModel->getListOfVideoByConditions($albumId, $sectionId, $reviewStatus, $pageSize);

        if ($result) {
            BasicTool::echoJson(1, "成功", $result);
        } else {
            BasicTool::echoJson(0, "获取视频列表失败");
        }
    } catch (Exception $e) {
