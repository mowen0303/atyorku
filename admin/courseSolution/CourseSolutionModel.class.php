...
class CourseSolutionModel extends Model
{
    ...
    function getSolutionById($id){
        $sql = "SELECT course_solution.*,user.alias,user.degree,user.gender,user.enroll_year,user.img as profile_img_url,user.major FROM (SELECT * FROM course_solution WHERE id = {$id}) AS course_solution INNER JOIN user on course_solution.answerer_user_id = user.id";
