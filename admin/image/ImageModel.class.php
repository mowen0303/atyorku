    ...
    function uploadImg($inputName, $userId, $maxFileSize = 1000000, $maxLength = 1000, $generateThumbnail = true, $maxThumbnailFileSize = 600000, $maxThumbnailLength = 200) {
        ...
        try {
            ...
            if ($file != null) {
                ...
                //返回上传文件名
                $arr = [];
                $arr["image_url"] = $uploadsDir . $newFileName;
                $arr["thumbnail_url"] = $thumbnailUploadsDir . $newFileName;
                $arr["height"] = $newHeight;
                $arr["width"] = $newWidth;
                $arr["size"] = filesize($root . $uploadsDir . $newFileName);
