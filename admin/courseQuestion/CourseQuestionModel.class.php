...
class CourseQuestionModel extends Model {
    ...
    function getQuestionById($id) {
        $sql = "SELECT course_question.*,user_class.is_admin,user_class.title AS user_title FROM (SELECT course_question.*,user.alias,gender,enroll_year,major,degree,user_class_id,user.img AS profile_img_url FROM (SELECT course_question.*,image.url AS img_url_3 FROM (SELECT course_question.*,image.url AS img_url_2 FROM (SELECT course_question.*,image.url AS img_url_1 FROM (SELECT * FROM course_question WHERE id in ({$id})) AS course_question LEFT JOIN image ON course_question.img_id_1 = image.id) AS course_question LEFT JOIN image ON course_question.img_id_2 = image.id) AS course_question LEFT JOIN image ON course_question.img_id_3 = image.id) AS course_question LEFT JOIN user ON course_question.questioner_user_id = user.id) AS course_question LEFT JOIN user_class ON course_question.user_class_id = user_class.id";
