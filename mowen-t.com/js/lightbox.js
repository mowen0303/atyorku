...
(function($) {
	...
	$.fn.lightBox = function(settings) {
		...
		settings = jQuery.extend({
			// 配置相关变量
			overlayBgColor: 		'#ccc',		// 背景颜色
			overlayOpacity:			0.9,		// 不透明值
			// 配置与导航
			fixedNavigation:		false,		// （布尔）导航（next和prev按钮）是否固定显示。
			// 配置相关的图像
			imageLoading:			'img/lightbox/lightbox-ico-loading.gif',		// （字符串）加载图标的图像路径
			imageBtnPrev:			'img/lightbox/lightbox-btn-prev.png',			// （字符串）上一页按钮的图像路径
			imageBtnNext:			'img/lightbox/lightbox-btn-next.png',			// （字符串）下一个按钮的图像路径
			imageBtnClose:			'img/lightbox/lightbox-btn-close.png',		// （字符串）关闭按钮的图像路径
			imageBlank:				'img/lightbox/lightbox-blank.gif',			// （字符串）一个像素的空白图像路径
			// 配置相关的集装箱图像框
			containerBorderSize:	15,			// （整数）如果你调整填充样式的容器，#lightbox-container-image-box，您将需要更新此值
			containerResizeSpeed:	20,		//（整数）指定时间容器的图像调整。这些数毫秒。400是默认。
			// 配置相关的文本标题。例如：2或8。你可以改变“图像”和“文本。
			txtImage:				'',	// （字符串）指定文本的“形象”
			txtOf:					'/',		// （字符串）指定文本”
			// 配置相关的键盘导航
			keyToClose:				'27',		// （字符串）（C =关闭）信关闭的jQuery灯箱接口。除了这封信，字母X和SCAPE关键是使用。
			keyToPrev:				'37',		// （字符串）（P =上一张）给显示上一张图片
			keyToNext:				'39',		// （字符串）（N =下一张）信显示下一张图片。
			// 以任何方式改变这些变量
			imageArray:				[],
			activeImage:			0
		},settings);
		...
		* @参数对象jQueryMatchedObj所有匹配的元素的jQuery对象
		...
		function _set_interface() {
			// 应用到body标签的HTML标记
			$('#container').append('<div id="jquery-overlay"></div><div id=lightboxmy><div id="jquery-lightbox"><div id="lightbox-container-image-box"><div id="lightbox-secNav"><a href="#" id="lightbox-secNav-btnClose"><img src="' + settings.imageBtnClose + '"></a></div><div id="lightbox-container-image"><img id="lightbox-image"><div style="" id="lightbox-nav"><a href="#" id="lightbox-nav-btnPrev"></a><a href="#" id="lightbox-nav-btnNext"></a></div><div id="lightbox-loading"><a href="#" id="lightbox-loading-link"><img src="' + settings.imageLoading + '"></a></div></div></div><div id="lightbox-container-image-data-box"><div id="lightbox-container-image-data"><div id="lightbox-image-details"><span id="lightbox-image-details-caption"></span><span id="lightbox-image-details-currentNumber"></span></div></div></div></div></div>');	
			// Get page sizes
			var arrPageSizes = ___getPageSize();
			// 样式覆盖，并显示它
			$("#lightboxmy").height(win_h).show();
			$('#jquery-overlay').css({
