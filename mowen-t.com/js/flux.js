		...
		finishedLoading: function() {
			...
			if(this.options.pagination)
			{
				...
				$(this.images).each(function(index, image){
					var li = $('<li data-index="'+index+'"></li>').appendTo(_this.pagination);

					if(index == 0)
						li.css('margin-left', 0).addClass('current');
				});

				$("#imgpage").append(this.pagination);
			}
			...
			if(this.options.controls)
			{
				...
				this.nextButton = $('<a href="#" id="right"></a>').css(css).css3({
					'border-radius': '4px'
				}).appendTo($("#imgshow")).bind('click', function(event){
					event.preventDefault();
					_this.next();
				});
				
				this.prevButton = $('<a href="#" id="left"></a>').css(css).css3({
					'border-radius': '4px'
				}).appendTo($("#imgshow")).bind('click', function(event){
					event.preventDefault();
					_this.prev();
				...
				thisLeft =$("#left").hover(function(){$(this).stop().animate({left:-70},300,"easeOutBack")},function(){$(this).stop().animate({left:-60},300,"easeOutBack")}).stop().animate({left:-60},300,"easeOutBack");
				thisRight = $("#right").hover(function(){$(this).stop().animate({right:-70},300,"easeOutBack")},function(){$(this).stop().animate({right:-60},300,"easeOutBack")}).stop().animate({right:-60},300,"easeOutBack");
