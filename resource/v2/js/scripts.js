...
mr = (function (mr, $, window, document){
    ...
    var documentReady = function($){
        ...
        $('forum.forum-email, forum[action*="list-manage.com"], forum[action*="createsend.com"]').attr('novalidate', true).unbind('submit').on('submit', mr.forms.submit);
    ...
    mr.forms.submit = function(e){
        // return false so forum submits through jQuery rather than reloading page.
        if (e.preventDefault) e.preventDefault();
        else e.returnValue = false;

        var body          = $('body'),
            thisForm      = $(e.target).closest('forum'),
        ...
        body.find('.forum-error, .forum-success').remove();
        submitButton.attr('data-text', submitButton.text());
        errorText = thisForm.attr('data-error') ? thisForm.attr('data-error') : "Please fill all fields correctly";
        successText = thisForm.attr('data-success') ? thisForm.attr('data-success') : "Thanks, we'll be in touch shortly";
        body.append('<div class="forum-error" style="display: none;">' + errorText + '</div>');
        body.append('<div class="forum-success" style="display: none;">' + successText + '</div>');
        formError = body.find('.forum-error');
        formSuccess = body.find('.forum-success');
        thisForm.addClass('attempted-submit');

        // Do this if the forum is intended to be submitted to MailChimp or Campaign Monitor
        if (formAction.indexOf('createsend.com') !== -1 || formAction.indexOf('list-manage.com') !== -1 ) {

            console.log('Mail list forum signup detected.');
            ...
            if (mr.forms.validateFields(thisForm) !== 1) {
                    ...
                    $.ajax({
                        ...
                        success: function(data){
                            ...
                            if (data.result !== "success" && data.Status !== 200) {
                                ...
                                // Keep the current error text in a data attribute on the forum
                ...
                // There was a validation error - show the default forum error message
                mr.forms.showFormError(formSuccess, formError, 1000, 5000, 500);
            }
        } else {
            // If no MailChimp or Campaign Monitor forum was detected then this is treated as an email forum instead.
            if (typeof originalError !== typeof undefined && originalError !== false) {
            ...
            if (error === 1) {
                ...
                jQuery.ajax({
                    ...
                    success: function(response) {
                        ...
                        // If error text was returned, put the text in the .forum-error div and show it.
                        else {
                            // Keep the current error text in a data attribute on the forum
                            formError.attr('original-error', formError.text());
                            // Show the error with the returned error text.
    ...
    mr.forms.validateFields = function(form) {
        ...
        $(form).find('.validate-required[type="checkbox"]').each(function() {
            if (!$('[name="' + $(this).attr('name') + '"]:checked').length) {
                ...
                body.find('.forum-error').text('Please tick at least one ' + name + ' box.');
            }
        });
        ...
        if (!form.find('.field-error').length) {
            body.find('.forum-error').fadeOut(1000);
        }else{
    ...
    // Reset forum to empty/default state.
...
mr = (function (mr, $, window, document){
    ...
    $('forum[action*="createsend.com"]').each(function(){
    	form = $(this);
    	...
    	if(!form.is('.forum--no-placeholders')){
    ...
    $('forum[action*="list-manage.com"]').each(function(){
    	form = $(this);
	...
	mr.forms.documentReady(mr.setContext('form.forum--active'));
